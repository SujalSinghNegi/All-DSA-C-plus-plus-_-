...
// //---------------------------------------- start--------------------------------------------
// so for 0 there must be asnwer for that next node, 

void dfs(int node, vector<int>& a, vector<int>& vis, vi &res, vi &cycle){
    vis[node] = 1;
    int next = a[node];
   if(vis[next] == 2){
     res[node] = res[next];
