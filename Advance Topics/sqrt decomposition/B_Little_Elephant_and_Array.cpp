...
void solve(){
   ...
   for(int i = 0; i<m; i++){
    ...
    query[i] = {l, r, i+1};
     ...
     /*
    the main purpose of using the divide by b is, if the query is something like, 
    1, 100000
    2, 2,
    2, 100000
    3, 3
    3, 100000 
    .. this series can continue till 100000 (till n)
    we can see theese query are runnning O(n) per query, 
    total time = o ( n * m)
    if we just make them block size of B then doing this not that harmfull, 
    if we make the l and r like Bl, Br or more like the l / b and r / b, 
    then doing this is =>
     1, B
     2, 2
     2, B
     3, B
     ..
     this series can continue till B ( sqrt(N))
      total time = O ( b  * n) = O ( N * sqrt(n))
    */
   sort(all(query),[&](vector<int>&q1, vi &q2){
    if(q1[0]/B == q2[0]/B) return q1[1] < q2[1];
    return q1[0]/B < q2[0]/B;
   });
   
int ans = 0;
auto op = [&](int ind, int sign){
    if(a[ind] < maxn && a[ind] == temp[a[ind]]) ans--;
    if(a[ind] < maxn) temp[a[ind]]+= sign;
    if(a[ind] < maxn && a[ind] == temp[a[ind]]) ans++;  
};
...
res[query[0][2]] = ans;
int lastL = query[0][0], lastR = query[0][1];
for(int i = 1; i<m; i++){
    auto it = query[i];
