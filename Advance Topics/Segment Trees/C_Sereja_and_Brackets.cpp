...
void build(int ind, int low, int high, string &arr){
    ...
    seg[ind].match = seg[2*ind+1].match+seg[2*ind+2].match;
    int mini = min(seg[2*ind+1].open, seg[2*ind+2].close);
    seg[ind].match += 2*mini;
    seg[ind].open = seg[2*ind+1].open- mini + seg[2*ind+2].open;
    seg[ind].close = seg[2*ind+1].close + seg[2*ind+2].close - mini;


}

node query(int ind, int low, int high, int l , int r){
    if(l>high || r < low) return {0, 0, 0};
    if(low >= l && high <= r) return seg[ind];
    int mid = low + (high - low) / 2;
    node left = query(2*ind+1, low, mid, l, r);
    node right= query(2*ind+2, mid+1, high, l, r);
    int mini = min(left.open, right.close);
    return {left.match + right.match + 2*mini, left.open+right.open-mini, left.close + right.close -mini};
}
