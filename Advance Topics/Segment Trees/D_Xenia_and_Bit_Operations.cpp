...
void build(ll ind, ll low, ll high, ll *arr){
    ...
    ll sz = high - low + 1;
    ll msb = 63 - __builtin_clzll(sz);
    if(msb&1) seg[ind] = (seg[2*ind+1] | seg[2*ind + 2]);
    else seg[ind] = (seg[2*ind + 1] ^ seg[2*ind + 2]);
}

ll query(ll ind, ll low, ll high, ll l , ll r){
    if(l>high || r < low) return 2e18;
...
void update(ll ind, ll low, ll high, ll i , ll val){
  //  cout << " i = " << i << " low  = " << low << " high = " << high << " val = " << val << endl;
    if(low == high) {
        seg[ind] = val;
        return;
    }
...
SGTree seg(sz);
seg.build(0, 0, sz-1, arr);
     while(m--){
        ll i , val ;
        cin>> i >> val;
        i--;
        seg.update(0, 0, sz-1, i, val);
      //  prllVector(seg.seg);
        cout<< seg.seg[0] << endl;
